 ...
 */
async function usingComponents(path, options = { globalComponents: [] }) {
  const fileContent = String(await readFile(path, 'utf8'))
  const tags = await customWxmlTags(fileContent)
  const customTags = options.globalComponents
    ? tags.filter(i => !options.globalComponents.includes(i))
    : tags
